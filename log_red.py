...
parser.add_argument("-c", "--cal_time", dest="cal_time", default=2, type=float)
...
parser.add_argument("-pc", "--plot_clip", dest="plot_clip", action="store_false")
...
def get_image_data_temperature(filenames,cal_time=2,spect_time=1e-2,file_time=5 ,decimation=1,
        win_size=32,tails=32, temperature=True):
    """
    filenames   :   List with the names of the plots
    cal_time    :   Calibration time at the begining of each file measured in seconds
    spect_time  :   Time between two spectra measure in seconds
    file_time   :   Complete time of each file in minutes
    tails       :   Amount of spectrum for truncate
    temperature :   Return the data in absolute temperature measure in Kelvin
    """
    sample = read_10gbe_data(filenames[0]) #class to read each log file
    sample_spect, header = sample.get_complete() # get spectrums and headers with metadata
    sample.close_file()
    
    ## This part is necessary to analyze the calibration data
    ...
    ## Until  here, we have the necessary information to get the Y-factor
    ...
    for i in range(0, len(filenames)):
        ...
        t_hot = 10**((ENR_ns-7.8/2)/10.)*t_load+t_load # Check the losses for get the t_hot  #temp ns on
# We will change this value to check the sensibility (originally 7.8)

        t_rx = (t_hot*P_load -t_load*P_hot)/(P_hot-P_load)
        Freq=np.linspace(1.2,1.8,2048) #To plot the noise temperature of the receptor
        plt.plot(Freq,t_rx)
        plt.ylim(0,700)
        plt.show()
        ...
        data = data[100:,:] #100 for 10ms of integration
        print(np.shape(data))
       ...
       #now we look at the clipping
        plt.plot(np.bitwise_and(header[:spect_size,1],2**0), label='clip_0')
        plt.plot(np.bitwise_and(header[:spect_size,1],2**1), label='clip_1')
        plt.plot(np.bitwise_and(header[:spect_size,1],2**2), label='clip_2')
        plt.plot(np.bitwise_and(header[:spect_size,1],2**3), label='clip_3')
        plt.legend()
        plt.show()
    ...
    data_new/= np.std(data_new[100:,:],axis=0) #100 for 10ms of integration
...
#cal_time should be 1 in ARTE
def plot_folder(folder_name, log_per_img=1, cal_time=2, file_time=5,spect_time=1e-2, plot_misc=True, decimation=1, mov_avg_size=32, tails=32, img_folder="log_img",plot_clip=True,plot_antenna=True):
    """
    
        This function is for plot the data. We can choose add more plots,
        like the antennas or the clipping over the spectrograms.

        folder_name  : Name of the folder that contain the log files.
        log_per_img  : Amout of logs we want to plot together.
        cal_time     : Calibration time measured in seconds.
        file_time    : Duration of each log file.
        spect_time   : Time each spectrums, measured in seconds.
        decimation   : Amount of spectrums we want to average.
        mov_avg_size : Window size.
    """
    ...
    for i in range(n_img):
        ...
        axes[0].axis(ymin =  np.median(avg_pow)-10, ymax = np.median(avg_pow)+10)
        axes[0].grid()
        ...
        #plt.show()#DELETE THIS LINE is just to see the plot
        plt.close()
