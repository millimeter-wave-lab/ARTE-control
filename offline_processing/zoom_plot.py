...
from log_reduction import get_dm_data
from log_red_V2v2f import get_image_data_temperature
import numpy as np
...
def zoom_plot(folder_name, start, stop=None, 
    ...
    if(stop is None):
        logs = [dirs[start_ind]]
        data, avg_pow, clip, t, bases,data_new, flags = get_image_data_temperature(
                logs,
                cal_time=cal_time, 
    ...
    data, avg_pow, clip, t, bases,flags,data_new = get_image_data_temperature(
            logs,
            cal_time=cal_time, 
            spect_time=spect_time,
            file_time=file_time, 
    ...
    if(temperature):
       ...
       # cmap = 'viridis', vmax = 160,vmin =  120,shading='auto'
        plt.pcolormesh(t[start_t:stop_t], freq[f_i:f_e] , 
                data[start_t:stop_t,f_i:f_e].T,cmap = 'viridis', vmax = 160,vmin =  120,shading='auto' )
    else:
        plt.pcolormesh(t[start_t:stop_t], freq[f_i:f_e] , 
                data[start_t:stop_t,f_i:f_e].T,cmap = 'viridis', vmax = 360,vmin =  120,shading='auto')
    plt.xlabel('minutes')
    plt.ylabel('MHz')
    cbar = plt.colorbar()
