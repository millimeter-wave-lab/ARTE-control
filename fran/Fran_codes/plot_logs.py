...
import simulate_pulse as simpulse
import sigpyproc.readers as readers

N_CHANNELS = 2048
FREQUENCIES = np.linspace(1.2, 1.8, N_CHANNELS, endpoint=False)[::-1] * apu.GHz

SAMPLE_RATE = 1 / (10 * apu.ms)  #TODO: confirm this value!
...
reference_frequency = FREQUENCIES[0].copy()
telescope = "ARTE"
...
def get_image_data_temperature(filenames,cal_time=1,spect_time=1e-2,file_time=5 ,decimation=15,
    ...
    return data, avg_pow, t, bases,flags, data_new, t_rx
...
def plot_folder(folder_name, log_per_img=1, cal_time=1, file_time=2,spect_time=1e-2,
    ...
    for i in range(n_img):
        ...
        delta = timedelta(hours = 4) # para convertir en utc
        d_i = d_i + delta
        d_f = d_f + delta
        # #
        ...
        fig, axes = plt.subplots(2,1 ,gridspec_kw={'height_ratios': [0.2,0.8]}, sharex=True)
        ...
        graph = axes[1].pcolormesh(t,freq , ((data_new[:len(t),::])).T, cmap = 'viridis',vmax =vmax,vmin =vmin,shading='auto' )
