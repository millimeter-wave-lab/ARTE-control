...
def get_image_data_temperature(filenames,cal_time=1,spect_time=1e-2,file_time=5 ,decimation=15,
    ...
    for i in range(0, len(filenames)):
        ...
        mediana = (np.median(data[30:,:],axis=0))
        #
        ###ACA CORRECCION DATA
        data = data[30:,:]
        #data = data[:,:]
    ...
    #data_new = np.delete(data_new,2000 , axis=1)
    print(data_new.shape)
    return data, avg_pow, clip, t, bases,flags, data_new
...
def plot_folder(folder_name, log_per_img=1, cal_time=1, file_time=2,spect_time=1e-2,
    ...
    for i in range(n_img):
        ...
        data, avg_pow, clip, t, bases,flags,data_new = get_image_data_temperature(sublogs,cal_time, spect_time,
        ...
        #
        '''
        d_i = datetime.strptime(hr_i, '%Y-%m-%d %H:%M:%S')
        d_f = datetime.strptime(hr_f, '%Y-%m-%d %H:%M:%S')
        delta = timedelta(hours = 3) # para convertir en utc en verano desde el 1 de sep
        #delta = timedelta(hours = 4) # para convertir en utc en invierno
        d_i = d_i + delta
        d_f = d_f + delta
        # #
        hr_i = d_i.strftime('%Y-%m-%d %H:%M:%S')
        hr_f = d_f.strftime('%Y-%m-%d %H:%M:%S')
        '''
        ...
        print('shape data new')
        data_new.shape


        std = np.std(data_new.flatten())*0.3
        print(std)

        data_new = np.where(data_new ==0, -1e7, data_new)
        graph = axes[1].pcolormesh(t,freq , ((data_new[:len(t),::])).T, cmap = 'viridis',vmax = 2,vmin = -2,shading='auto' )


        # graph = axes[1].pcolormesh(t,freq , ((data_new[:len(t),::])+np.mean(avg_pow_int)).T, cmap = 'viridis',vmax = std+np.mean(avg_pow),vmin = -std+np.mean(avg_pow),shading='auto' )
                                   # vmax = 245,vmin =  135,shading='auto' )
