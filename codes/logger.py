...
#noise_params, first_amp_params and lna_params are not present in logger_really_used_santamartina.py, but that are parameters to set the Rigol.
...
## All this next code is not in logger_really_used_santamartina.py
...
#All this next code is not in logger_really_used_santamartina.py
        ...
        #rigol_file.write(str(time.time())+'\n')
        #roach_control.enable_diode() esta mal aca



        #status = not(rigol.get_status(noise_params[0]))
        #rigol_file.write('Estado hot(True->ON)  '+str(status)+' '+filename+'\n')
        #current_NS_hot = rigol.meas_current(noise_params[0])
        #voltage_NS_hot = rigol.meas_voltage(noise_params[0])
        roach_control.enable_diode()
        #time.sleep(cal_time//2)
        ...
        #status = not(rigol.get_status(noise_params[0]))
        #rigol_file.write('Estado cold(False->OFF)  '+str(status)+' '+filename+'\n')
        #current_NS_cold = rigol.meas_current(noise_params[0])
        voltage_NS_cold = rigol.meas_voltage(noise_params[0])
        #if(rigol.get_status(noise_params[0])):
            #raise Exception('Channel %i doesnt turn off!'%noise_params[2])
        time.sleep(cal_time//2)
        roach_control.disable_diode()
        #rigol_file.write('Corriente cold:  '+ str(current_NS_cold)+'A '+'\n')
        #rigol_file.write('Voltaje cold:  '+ str(voltage_NS_cold)+'V '+'\n')
        #rigol_file.write('Corriente hot:  '+ str(current_NS_hot)+'A '+'\n')
        #rigol_file.write('Voltaje hot:  '+ str(voltage_NS_hot)+'V '+'\n')
